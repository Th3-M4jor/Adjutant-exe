defmodule BnBBot.RoleBtn do
  ...
  def handle_role_btn_click(%Nostrum.Struct.Interaction{} = inter, id) do
    ...
    add_or_remove = unless Enum.member?(inter.member.roles, id) do
      {:ok} = Api.add_guild_member_role(inter.guild_id, inter.member.user.id, id, "Button Click")
      "added"
    else
      {:ok} = Api.remove_guild_member_role(inter.guild_id, inter.member.user.id, id, "Button Click")
      "removed"
    end
